...
export default function Profile() {
  ...
  const [profile, setProfile] = useState<FarmerProfile>(
    () => ProfileService.get() || ProfileService.getDefault()
  );
  ...
  return (
    <main className="container px-4 py-10 max-w-2xl">
      <div className="flex items-start justify-between mb-2 flex-wrap gap-2">
        <h1 className="text-3xl font-heading font-bold text-foreground">{t.profile.title}</h1>
        <SyncIndicator />
      </div>
      ...
      <div className="space-y-6">
        <div>
          ...
          <Input value={profile.name} onChange={(e) => update("name", e.target.value)} className="mt-2 h-14 text-lg bg-card" />
        </div>

        <div>
          <Label className="text-base font-semibold">{t.profile.state}</Label>
          <Select value={profile.state} onValueChange={(v) => update("state", v)}>
            <SelectTrigger className="mt-2 h-14 text-lg bg-card"><SelectValue placeholder={t.selectState} /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Input value={profile.district} onChange={(e) => update("district", e.target.value)} className="mt-2 h-14 text-lg bg-card" />
        </div>

        <div>
          <Label className="text-base font-semibold">{t.profile.landSize}</Label>
          <Input type="number" value={profile.landSize || ""} onChange={(e) => update("landSize", Number(e.target.value))} className="mt-2 h-14 text-lg bg-card" />
        </div>
        ...
        <div>
          ...
          <Select value={profile.landType} onValueChange={(v: any) => update("landType", v)}>
            <SelectTrigger className="mt-2 h-14 text-lg bg-card"><SelectValue placeholder={t.selectLandType} /></SelectTrigger>
            <SelectContent>
              {(["irrigated", "rainfed", "dryland", "wetland"] as const).map((lt) => (
                <SelectItem key={lt} value={lt}>{t.landTypes[lt]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Input type="number" value={profile.annualIncome || ""} onChange={(e) => update("annualIncome", Number(e.target.value))} className="mt-2 h-14 text-lg bg-card" />
        </div>
        ...
        <div>
          ...
          <Select value={profile.category} onValueChange={(v: any) => update("category", v)}>
            <SelectTrigger className="mt-2 h-14 text-lg bg-card"><SelectValue placeholder={t.selectCategory} /></SelectTrigger>
            <SelectContent>
        ...
        <div className="flex items-center justify-between bg-card rounded-xl p-4 border">
          ...
          <Switch checked={profile.irrigationAvailable} onCheckedChange={(v) => update("irrigationAvailable", v)} />
        </div>

        <div className="flex items-center justify-between bg-card rounded-xl p-4 border">
          <Label className="text-base font-semibold">{t.profile.bankLinked}</Label>
          <Switch checked={profile.bankLinked} onCheckedChange={(v) => update("bankLinked", v)} />
        </div>
